...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	spentCalories := (runningCaloriesMeanSpeedMultiplier * speed * runningCaloriesMeanSpeedShift) * weight / mInKm *
		duration * minInH
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	meanSpeedInMSec := meanSpeed(action, duration) * kmhInMsec //Средняя скорость в М/С
	heightInM := height / cmInM                                //Рост в метрах
	spentCalories := (walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeedInMSec, 2)/heightInM)*
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	meanSpeedInSwimming := swimmingMeanSpeed(lengthPool, countPool, duration) //Средняе скорость при плавание
	spentCaloriesInSwimming := (meanSpeedInSwimming + swimmingCaloriesMeanSpeedShift) *
		swimmingCaloriesWeightMultiplier * weight * duration
	return spentCaloriesInSwimming
}
